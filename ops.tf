resource "github_repository" "ops" {
  name         = "ops"
  description  = "Terraforming the world to have more social kites 🌏🪁🪁"
  homepage_url = "https://buildkite.com/kitesocial/ops"
  topics       = ["buildkite", "terraform"]
...
data "aws_ssm_parameter" "buildkite-agent-ops-token" {
  name = "/kitesocial/ops/buildkite-agent/BUILDKITE_AGENT_TOKEN"
}

resource "aws_cloudwatch_log_group" "buildkite-agent-ops" {
  name = "/kitesocial/ops/buildkite-agent"
}
